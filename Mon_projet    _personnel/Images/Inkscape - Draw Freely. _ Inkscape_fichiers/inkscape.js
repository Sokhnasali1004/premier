...
$(document).ready(function() {
  setupMenu(false);
  setupInlinePages();
  $("[src$='.svg']").on("error", fallbackToPNG);
  $(".image.only img").on("error", fallbackToIcon);
...
function onTouch(event) {
  withtouch = true;
  $("body")[0].removeEventListener('click', onTouch);
}
$(window).on( "load", function() {
  $(document).ready(function() {
    withtouch = false
    $("body")[0].addEventListener("touchstart", onTouch);
    furnishShieldTabs();
    furnishShieldTabsTouch();
...
function fallbackToIcon(event) {
  this.src = $(this).data('icon');
}

function fallbackToPNG(event) {
...
var block = false;
function furnishShieldTabsTouch() {
  $("#shield > .tabs").find("a").each(function(i, el) {
      $(el)[0].addEventListener("touchstart", function(event) {
        $("#shield > .tabs > li.current").each(function(i,el) {
...
function furnishShieldTabs() {
  if(screen.width >= 960) {
    ...
    var banners = $("#shield > .tabs").children("li");
    var pos = 0;
    selectBanner($(banners[pos])); 
    this.sb_timer = setInterval(
      function() { 
        if (block) {
          return true;
        }
        ...
        var nextTab = $(banners[pos]);
        selectBanner(nextTab); 
      }, 3000);
  }
    ...
    if(document.getElementById('blanket')) {
      ...
      if(note) {
        if(Array.isArray(note)) {
          $('#popup .buttons').append("<textarea name='" + note[2] + "' placeholder='" + note[0] + "'>"+note[1]+"</textarea>");
        } else {
...
function setupMenu(refreshwin) {
        ...
        if($(this).children("a").attr('href')) {
          ...
          $(this).children("a").click(function(event) {
            ...
            if(withtouch) {
              ...
              $(this).parent().children("ul").css("display","inline-block");
            }
          });
        } else {
          $(this).children("a").click(function(event) {
  ...
  if(screen.width >= 960) {
    ...
    expectedWidth = expectedWidth * 1.25;
    var fontSize = 18, minFontSize = 1;
    var paddingSize = 75, paddingFactor = 0.85;
    do {
      $("#menu > li > a").css({
